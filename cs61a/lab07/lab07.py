...
def subseqs(s):
    ...
    if s == []:
        ...
        # Leap of faith
        sub_seqs_without_first = subseqs(s[1:])
        # Solve the full problem
        return sub_seqs_without_first + insert_into_all(s[0], sub_seqs_without_first)
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            # Prev is already in sequence
            return subseq_helper(s[1:], prev)
        else:
            a = subseq_helper(s[1:], prev) # The case where the prefix does not have s0
            b = subseq_helper(s[1:], s[0]) # The case where the prefix is s0
