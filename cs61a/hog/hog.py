...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        result = dice()
        sum, num_rolls = sum + result, num_rolls - 1
        if result == 1:
            one = result
    if one == 1:
        sum = 1
...
def free_bacon(score):
    ...
    "*** YOUR CODE HERE ***"
    one_digit = score % 10
    ten_digit = (score - one_digit) // 10 % 10
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            latest_high = score0 - last_score
            if latest_high > current_high:
                print(latest_high, 'point(s)! That\'s the biggest gain yet for Player', who)
                current_high = latest_high
            return announce_highest(who, score0, current_high)
        else:
            current_high = running_high
            latest_high = score1 - last_score
            if latest_high > current_high:
                print(latest_high, 'point(s)! That\'s the biggest gain yet for Player', who)
                current_high = latest_high
...
def make_averaged(original_function, trials_count=1000):
    ...
    def cal_average(*args):
        ...
        while total_count > 0:
            total_count, sum = total_count - 1, sum + original_function(*args)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while dice_number <= 10:
        average_score = make_averaged(roll_dice, trials_count)(dice_number, dice)
        if average_score > highest_average_score:
            best_number, highest_average_score = dice_number, average_score
        elif average_score == highest_average_score:
            if best_number > dice_number:
                best_number = dice_number
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    opponent_score_one_digit = opponent_score % 10
    opponent_score_ten_digit = opponent_score // 10 % 10
    give = 10 - opponent_score_one_digit + opponent_score_ten_digit
    if give >= cutoff:
        return 0
    else:
        return num_rolls
