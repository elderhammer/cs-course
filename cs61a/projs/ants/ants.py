...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while check_place is not beehive and index <= self.max_range:
            if self.min_range <= index:
                some_bee = rANTdom_else_none(check_place.bees)
                if some_bee is not None:
                    return some_bee
            check_place, index = check_place.entrance, index + 1
        return None
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting > 0:
            ...
            some_bee = rANTdom_else_none(self.place.bees)
            if some_bee is not None:
                self.eat_bee(some_bee)
            ...
            while check_place is not None:
                if check_place.ant is not None:
                    ...
                    if isinstance(ant, ContainerAnt):
                        if ant.contained_ant is not None:
                            if ant.contained_ant.has_been_inspired == False:
                                ant.contained_ant.damage *= 2
                                ant.contained_ant.has_been_inspired = True
                    if ant.has_been_inspired == False:
                        ant.damage *= 2
                        ant.has_been_inspired = True
