...
def make_guess(n):
    ...
    4
    """
    # Functions use lexical scope to pass state
    def update_guess(num_incorrect):
        # Return the function name directly, and then look at the function definition when it is called.
        def new_guess(x):
            if x == n:
                # When calling, find the name binding according to the parent environment.
